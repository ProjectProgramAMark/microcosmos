...
class TurboWorm(Swimmer):
    """5x worm speed, variable rest lengths (0.3–2.0), high frequency.

    Designed to stress-test LBM stability with extreme velocity + non-uniform chain.
    """

    swimmer_name = 'turbo_worm'
    plot_color = 'crimson'

    def setup(self):
        self.num_nodes = math.floor(self.cfg.experiment.num_nodes * 0.6)
        base_rotation_speed = self.cfg.experiment.get('rotation_speed', 0.05)
        self.rotation_speed = base_rotation_speed * 5.0
        self.num_wavelengths = 3  # high spatial frequency
        ...
        # Variable rest lengths: sweep from 0.3 to 2.0 along body
        self.rest_lengths = jnp.linspace(0.3, 2.0, num_edges)

        # Place nodes so initial spacing matches rest lengths
        domain_height, domain_width = self.grid_shape
        cumulative = jnp.concatenate([jnp.zeros(1), jnp.cumsum(self.rest_lengths)])
        total_length = cumulative[-1]

        # Center chain horizontally, leave 10% margin each side
        available = domain_width * 0.8
        scale_factor = available / total_length
        x_positions = domain_width * 0.1 + cumulative * scale_factor

        # Sine wave along body with high frequency
        ...
        # Override _make_edges to use variable rest lengths
