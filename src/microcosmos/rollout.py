...
def _identity_valid(previous: EcosystemState, state: EcosystemState) -> jax.Array:
    ...
    ids_in_range = jnp.all(
        jnp.where(
            alive,
            (population.individual_id >= 0)
            & (population.individual_id < population.next_individual_id),
            True,
        )
    )
    return (
        ~duplicated
        & ids_in_range
        & (population.next_individual_id >= previous.population.next_individual_id)
    )
...
def initialize_chunk_metrics(state: EcosystemState) -> EcosystemChunkMetrics:
    ...
    live_energy = jnp.min(
        jnp.where(state.population.alive, state.population.energy, jnp.inf)
    )
    ...
    return EcosystemChunkMetrics(
        ...
        maximum_resource_excess=jnp.max(
            state.fields.energy - state.resource_capacity_map
        ),
...
def update_chunk_metrics(
    ...
    births_valid = (
        telemetry.birth_count == jnp.sum(telemetry.birth_child_ids >= 0)
    ) & (telemetry.birth_count == jnp.sum(telemetry.birth_parent_ids >= 0))
    ...
    return EcosystemChunkMetrics(
        ...
        maximum_individual_id=jnp.maximum(
            metrics.maximum_individual_id, jnp.max(state.population.individual_id)
        ),
        minimum_resource=jnp.minimum(
            metrics.minimum_resource, jnp.min(state.fields.energy)
        ),
        ...
        finite=metrics.finite & _all_finite(state),
...
def combine_chunk_metrics(
    ...
    return EcosystemChunkMetrics(
        ...
        maximum_individual_id=jnp.maximum(
            first.maximum_individual_id, second.maximum_individual_id
        ),
        minimum_resource=jnp.minimum(
            first.minimum_resource, second.minimum_resource
        ),
        maximum_resource_excess=jnp.maximum(
            first.maximum_resource_excess, second.maximum_resource_excess
        ),
        minimum_live_energy=jnp.minimum(
            first.minimum_live_energy, second.minimum_live_energy
        ),
...
def run_ecosystem_chunk(env, state: EcosystemState, keys: jax.Array, action=None):
    ...
    (final_state, metrics), _ = jax.lax.scan(
        scan_step, (state, initial_metrics), keys
    )
...
def run_ecosystem_chunk_with_snapshots(
    ...
    def block_step(carry, block_keys):
        ...
        next_state, block_metrics = run_ecosystem_chunk(
            env, current, block_keys, action
        )
    ...
    (final_state, metrics), snapshots = jax.lax.scan(
        block_step, (state, initial_metrics), blocks
    )
